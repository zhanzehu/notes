...
public class MXText extends androidx.appcompat.widget.AppCompatEditText implements TextWatcher {
    ...
    public MXText(Context context) {
        super(context);
        init(null);
    }

    public MXText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public MXText(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs) {
        ...
        bulletRadius = array.getDimensionPixelSize(R.styleable.MXText_bulletRadius, 0);
        bulletGapWidth = array.getDimensionPixelSize(R.styleable.MXText_bulletGapWidth, 0);
        historyEnable = array.getBoolean(R.styleable.MXText_historyEnable, true);
        historySize = array.getInt(R.styleable.MXText_historySize, 100);
        linkColor = array.getColor(R.styleable.MXText_linkColor, 0);
        linkUnderline = array.getBoolean(R.styleable.MXText_linkUnderline, true);
        quoteColor = array.getColor(R.styleable.MXText_quoteColor, 0);
        quoteStripeWidth = array.getDimensionPixelSize(R.styleable.MXText_quoteStripeWidth, 0);
        quoteGapWidth = array.getDimensionPixelSize(R.styleable.MXText_quoteCapWidth, 0);
        array.recycle();
    ...
    protected void styleInvalid(int style, int start, int end) {
        ...
        List<MXCursor> list = new ArrayList<>();

        for (StyleSpan span : spans) {
            if (span.getStyle() == style) {
                list.add(new MXCursor(getEditableText().getSpanStart(span), getEditableText().getSpanEnd(span)));
                getEditableText().removeSpan(span);
            }
        }

        for (MXCursor mxCursor : list) {
            if (mxCursor.isValid()) {
                if (mxCursor.getStart() < start) {
                    styleValid(style, mxCursor.getStart(), start);
                }

                if (mxCursor.getEnd() > end) {
                    styleValid(style, end, mxCursor.getEnd());
                }
            }
        }
    }
    ...
    protected void underlineInvalid(int start, int end) {
        ...
        for (MXCursor mxCursor : list) {
            if (mxCursor.isValid()) {
                if (mxCursor.getStart() < start) {
                    underlineValid(mxCursor.getStart(), start);
                }

                if (mxCursor.getEnd() > end) {
                    underlineValid(end, mxCursor.getEnd());
                }
            }
        }
    }
    ...
    protected void strikethroughInvalid(int start, int end) {
        ...
        for (MXCursor mxCursor : list) {
            if (mxCursor.isValid()) {
                if (mxCursor.getStart() < start) {
                    strikethroughValid(mxCursor.getStart(), start);
                }

                if (mxCursor.getEnd() > end) {
                    strikethroughValid(end, mxCursor.getEnd());
                }
            }
        }
    }
    ...
    protected void bulletValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (bulletStart < bulletEnd) {
                getEditableText().setSpan(new MXBulletSpan(bulletColor, bulletRadius, bulletGapWidth), bulletStart, bulletEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void quoteValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (quoteStart < quoteEnd) {
                getEditableText().setSpan(new MXQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), quoteStart, quoteEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void linkValid(String link, int start, int end) {
        ...
        linkInvalid(start, end);
        getEditableText().setSpan(new MXURLSpan(link, linkColor, linkUnderline), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
    ...
    public void fromHtml(String source) {
        ...
        switchToMXStyle(builder, 0, builder.length());
        setText(builder);
    }
    ...
    protected void switchToMXStyle(Editable editable, int start, int end) {
        ...
        for (BulletSpan span : bulletSpans) {
            ...
            editable.setSpan(new MXBulletSpan(bulletColor, bulletRadius, bulletGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (QuoteSpan span : quoteSpans) {
            ...
            spanEnd = 0 < spanEnd && spanEnd < editable.length() && editable.charAt(spanEnd) == '\n' ? spanEnd - 1 : spanEnd;
            editable.removeSpan(span);
            editable.setSpan(new MXQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (URLSpan span : urlSpans) {
            ...
            int spanEnd = editable.getSpanEnd(span);
            editable.removeSpan(span);
            editable.setSpan(new MXURLSpan(span.getURL(), linkColor, linkUnderline), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }
}
